...
class FreightBooking(models.Model):
    ...
    direction = fields.Selection(([('import', 'Import'),
                                   ('export', 'Export')]), string='Transfer', default='export')
    ...
    transport = fields.Selection(([('air', 'Air'),
                                   ('ocean', 'Ocean'),
                                   ('land', 'Land')]),
                                 string='Transport', default='ocean')
    ...
    sale_id = fields.Many2one('sale.order', string='Sale Order', copy=False, tracking=True)
    ...
    first_notify_id = fields.Many2one(
        'res.partner', string="1st Notify",)
    second_notify_id = fields.Many2one(
        'res.partner', string="2nd Notify",)
    
    container_types = fields.Selection(
        [('refer', 'Refer'),
         ('not_refer', 'Not Refer')],
        default='refer',
    )
