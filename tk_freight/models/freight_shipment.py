...
class FreightShipment(models.Model):
    ...
    # Odoo 19: use fields.Datetime.now (callable) instead of fields.Datetime.now() (evaluated at class load)
    ...
    policy_name = fields.Char(string='Policy Name', translate=True)
    ...
    def _onchange_tracking_template(self):
        for rec in self:
            if rec.tracking_template_id:
                rec.freight_log = [fields.Command.clear()]
                lines = []
    ...
    # Odoo 19: _read_group_stage_ids signature changed —
    # domain & order are no longer passed; stages is a recordset.
    @api.model
    def _read_group_stage_ids(self, stages, domain):
        return self.env['freight.shipment.stages'].search([])
    ...
    def _onchange_freight_packages(self):
        for rec in self:
            text = ""
            for data in rec.freight_packages:
                text += str(data.qty) + " X " + str(data.package.name) + \
                    " - " + str(data.name) + "\n"
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('name', 'New') == 'New':
                ...
                if transport == 'air':
                    ...
                    pre = str(air_prefix) if air_prefix else "AIR"
                    values['name'] = pre + (self.env['ir.sequence'].next_by_code('operation.master') or _('New'))
                elif transport == 'ocean':
                    ocean_prefix = self.env['ir.config_parameter'].sudo().get_param('tk_freight.ocean_seq')
                    pre = str(ocean_prefix) if ocean_prefix else "OCEAN"
                    values['name'] = pre + (self.env['ir.sequence'].next_by_code('operation.house') or _('New'))
                elif transport == 'land':
                    land_prefix = self.env['ir.config_parameter'].sudo().get_param('tk_freight.land_seq')
                    pre = str(land_prefix) if land_prefix else "LAND"
                    values['name'] = pre + (self.env['ir.sequence'].next_by_code('operation.direct') or _('New'))
            if values.get('name') and not values.get('tracking_number'):
                values['tracking_number'] = values.get('name')
        ...
        for rec in records:
            route_vals_base = {
                ...
                'shipment_id': rec.id,
                'pickup_datetime': rec.pickup_datetime,
                'arrival_datetime': rec.arrival_datetime,
                'address_to': rec.address_to,
                'shipper_id': rec.shipper_id.id,
                'consignee_id': rec.consignee_id.id,
                'charge_type': 'f',
                'inland_shipment_type': rec.inland_shipment_type,
                'ocean_shipment_type': rec.ocean_shipment_type,
            }
            if rec.transport == 'air':
                route_vals_base.update({'mawb_no': rec.mawb_no,
                                        'airline_id': rec.airline_id.id,
                                        'flight_no': rec.flight_no})
            elif rec.transport == 'ocean':
                route_vals_base.update({'vessel_id': rec.vessel_id.id,
                                        'obl': rec.obl})
            elif rec.transport == 'land':
                route_vals_base.update({'truck_ref': rec.truck_ref,
                                        'trucker': rec.trucker.id,
                                        'trucker_number': rec.trucker_number})
            if rec.transport in ('air', 'ocean', 'land'):
                route_id = self.env['freight.route'].create(route_vals_base)
    ...
    def compute_freight_port(self):
        for rec in self:
            ...
            if rec.transport == 'air':
                ids = ports.search([('air', '=', True)]).ids
            elif rec.transport == 'ocean':
                ids = ports.search([('ocean', '=', True)]).ids
            elif rec.transport == 'land':
                ids = ports.search([('land', '=', True)]).ids
            else:
                ids = []
            rec.port_ids = ids
    ...
    def _compute_service_charges(self):
        for rec in self:
            total = vendor_total = 0.0
            for data in rec.freight_services:
                if data.service_type in ('shipper', 'consignee'):
                    total += data.sale * data.qty
                else:
                    vendor_total += data.sale * data.qty
    ...
    def _compute_route_charges(self):
        for rec in self:
            total = sum(
                d.total_charge for d in rec.freight_routes if d.charge_type == 'p'
            )
    ...
    def _compute_custom_check(self):
        for rec in self:
            if rec.custom_ids:
                rec.pass_state = all(d.state == 'pass' for d in rec.custom_ids)
            else:
        ...
        for rec in self:
            if rec.total_charge and rec.policy_no and rec.policy_name and rec.date:
                ...
                self.freight_services.create({
                    'service_id': self.env.ref('tk_freight.policy_product_1').id,
    ...
    def _compute_final_charges(self):
        for rec in self:
            rec.final_charges = sum(d.qty * d.charges for d in rec.freight_packages)

    def action_add_shipment(self):
        record = ""
        for rec in self:
            for data in rec.freight_packages:
                ptype = 'Box / Cargo' if data.package_type == "item" else "Container / Box"
                record += "{} - {} - {} \n".format(ptype, data.name, data.package.name)
    ...
    def action_add_route(self):
        ...
        for rec in self:
            for data in rec.freight_routes:
                record += "{} - {}\n ".format(data.transport, data.name)
        self.route_add = True
        self.freight_services.create({
    ...
    def action_create_invoice(self):
        ...
        shipper_list = self.env['freight.service'].search(
            [('shipment_id', '=', self.id),
             ('service_type', '=', 'shipper'),
             ('invoiced', '=', False)]).mapped('shipper_id').ids
        consignee_list = self.env['freight.service'].search(
            [('shipment_id', '=', self.id),
             ('service_type', '=', 'consignee'),
             ('invoiced', '=', False)]).mapped('consignee_id').ids
        currency_list = self.env['freight.service'].search(
            [('shipment_id', '=', self.id),
             '|', ('service_type', '=', 'shipper'),
             ('service_type', '=', 'consignee'),
             ('vendor_invoiced', '=', False)]).mapped('currency_id').ids
                ...
                for c in currency_list:
                    for data in self.freight_services:
                        if data.service_type == "shipper" and not data.invoiced \
                            ...
                            data.status = "invoice"
                            data.invoiced = True
                            shipper_invoice_line.append((0, 0, {
                                'product_id': data.service_id.id,
                                'name': data.name,
                                'quantity': data.qty,
                                'price_unit': data.sale,
                            }))
                    if shipper_invoice_line:
                ...
                for c in currency_list:
                    for data in self.freight_services:
                        if data.service_type == "consignee" and not data.invoiced \
                            ...
                            data.status = "invoice"
                            data.invoiced = True
                            consignee_invoice_line.append((0, 0, {
                                'product_id': data.service_id.id,
                                'name': data.name,
                                'quantity': data.qty,
                                'price_unit': data.sale,
                            }))
                    if consignee_invoice_line:
                        self.env['account.move'].create({
    ...
    def action_create_vendor_bill(self):
        ...
        vendor_list = self.env['freight.service'].search(
            [('shipment_id', '=', self.id),
             ('service_type', '=', 'vendor'),
             ('vendor_invoiced', '=', False)]).mapped('vendor_id').ids
        currency_list = self.env['freight.service'].search(
            [('shipment_id', '=', self.id),
             ('service_type', '=', 'vendor'),
             ('vendor_invoiced', '=', False)]).mapped('currency_id').ids
    ...
    @api.onchange('address_to', 'source_location_id', 'destination_location_id',
    ...
    def action_create_shipper_invoice(self):
        ...
        }) for data in self.env['freight.shipment'].browse(self.ids)]
        invoice_id = self.env['account.move'].create({
            'partner_id': shipper_id,
            'invoice_line_ids': invoice_lines,
            'move_type': 'out_invoice',
        })
        self.env['freight.multiple.invoice'].create({
            'partner_id': shipper_id,
